...
Recommended next task: fill `research/sources.md` and `research/etecsa_network.md` with dated source citations for current ETECSA bands, then perform first local signal measurements using `data/templates/blackout_measurement.csv`. Record the minimum equipment and approximate cost for each successful path.
