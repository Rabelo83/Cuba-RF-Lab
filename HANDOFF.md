...
Recommended next task: redesign the Yagi director length taper and spacing in `calculations/preliminary_antenna_geometry.py` (proper taper and non-uniform, closer spacing, not uniform wavelength scaling), then re-run `simulations/nec/run_first_pass_yagi.py` against the revised geometry. Continue improving official import-rule and current ETECSA/regulatory source coverage in parallel. Do not wait for local measurements to begin simulation, but do not call any design field validated without them.
