...
- Local solver check on 2026-08-09 found no `xnec2c`, `nec2c`, or `openEMS` installed.
...
Recommended next task: choose or install an EM solver, generate first NEC/openEMS model files from the pre-simulation geometry seeds, and run the 900 MHz 5-element Yagi versus 1800 MHz 5-element Yagi comparison first. Continue improving official import-rule and current ETECSA/regulatory source coverage in parallel. Do not wait for local measurements to begin simulation, but do not call any design field validated without them.
