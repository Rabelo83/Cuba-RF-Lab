...
Recommended next task: generate the first Python `necpp` sweep scripts from the pre-simulation geometry seeds, and run the 900 MHz 5-element Yagi versus 1800 MHz 5-element Yagi comparison first. Continue improving official import-rule and current ETECSA/regulatory source coverage in parallel. Do not wait for local measurements to begin simulation, but do not call any design field validated without them.
