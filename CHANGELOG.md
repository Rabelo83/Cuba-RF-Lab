...
Replaced the failed director geometry with an NEC-driven local search (`simulations/nec/optimize_yagi_directors.py`, SciPy Nelder-Mead using `necpp` as the objective function) instead of a second hand-derived guess. Found working candidates for both bands: `Y900_5EL_OPT_V2_BW` and `Y1800_5EL_OPT_V2_BW` show flat 11.1-11.5 dBi simulated gain across their full required bands, close to or exceeding the project's original placeholder assumptions. Both still need a matching network before their native impedance is usable with ordinary coax. A real bug in an intermediate version of the optimizer (silently rescaling the antenna's physical size per test frequency) was caught by cross-checking against an independent re-simulation and fixed before any result was trusted. Updated the desktop link-budget scenarios, antenna candidate worksheet, and public Research page with the real simulated numbers.
